...
여러 GPU 박스를 옮겨다니면 매번 pip 설치가 번거롭습니다. **deps 를 구운 이미지**를 한 번 만들어
vast.ai 템플릿으로 쓰면 설치를 통째로 건너뜁니다. (모델은 여전히 런타임 다운로드 — 라이선스/용량 때문에 안 구움.)

**빌드(로컬 docker 불필요)**: GitHub Actions 가 클라우드에서 빌드 → `ghcr.io` 로 push.
1. GitHub → **Actions** 탭 → **build-image** → **Run workflow** (또는 `Dockerfile`/`install.sh` 푸시 시 자동).
2. 끝나면 패키지가 `ghcr.io/<유저>/gemma-vllm-speedtest:latest` 에 생김.
3. GitHub → 그 패키지 → **Package settings → Change visibility → Public** (vast.ai 가 자격증명 없이 pull 하도록).

**vast.ai 에서 사용**: 템플릿 이미지에 `ghcr.io/<유저>/gemma-vllm-speedtest:latest` 입력 → 부팅 후:
```bash
cd /opt/gemma-vllm-speedtest
HF_TOKEN=hf_xxx ./run.sh both      # SKIP_INSTALL 은 이미지에 박혀 있음(설치 스킵)
```
> 모델 다운로드 느림은 이미지로 못 고침(호스트 네트워크 문제) → 네트워크 좋은 호스트 고르거나
> vast.ai 영속 볼륨을 `HF_HOME` 에 마운트해 캐시 재사용. (deps 설치/flashinfer 사고는 이미지로 사라짐.)
